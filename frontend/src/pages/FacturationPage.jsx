...
import { hasPermission } from '../contexte/permissions';
    ...
    const fetchParamétrages = async (vale) => {
      ...
      try {
        ...
        console.log('Erreur lors de la récupération des paramètres :');
      } 
    };
    ...
    const fetchNumeroFacture = async (id) => {
      try {
        ...
        console.log("Erreur lors de la récupération du numéro de facture :");
        return null;
      }
    };
...
const FacturationPage = ( ) => {
  ...
  useEffect(() => {
    const recuperePara = async () =>{
      ...
      console.log('paramétrage para.enableTTC ', para.enableTTC)
      setParametrage(para)
  ...
  const transformToInvoiceCancel = (record)=>{
    setFacturePaye([])
    formRef.current?.resetFields(); // Réinitialise le formulaire
    setModaleFacturePayeVisible(!modaleFacturePayeVisible)
  }
  ...
  const handleOpenModal = (record, type) => {
    ...
    ReactDOM.render(<ModeleFacture parametrage={parametrage} factures={record} tvas={tvas} type={type}/>, tempDiv);
...
const handleModalOk = async () => {
  ...
  const handleEditInvoice = (record, type) => {
    ...
    if(record.statut === 'payée'){
      ...
      // Gestion de la taxe secondaire lors de l'édition
      if (record.taxe_secondaire && parseFloat(record.taxe_secondaire) > 0) {
        setUseTaxeSecondaire(true);
        setEditingTaxeSecondaire({
  ...
  const calculateTotals = () => {
    ...
    if (useTaxeSecondaire) {
      if (isEditingFacture && editingTaxeSecondaire) {
        // Mode modification : utiliser le taux de la facture originale
        taxeSecondaire = {
          label: `TPS`,
      ...
      render: (_, record) => {
        const menuItems = [
          record.statut !== 'payée'
            ? {
                ...
                onClick: () => transformToInvoiceOpen(record),
              }
            : {
                key: '2',
                label: 'Afficher le mode de règlement',
                onClick: () => transformToInvoiceOpen(record),
              },
          ...(record.statut !== 'payée'
            ? [
                {
                  ...
                  onClick: () => transformToInvoice(record, 'en attente'),
                },
              ]
            : []),
        ...
        return (
          <div className="actions">
            ...
            <Button className='ayeOnMobile' type="link" icon={<img src={eye} className='iconListe' onClick={()=>handleModalModeleFactureCancel(record, 'FACT')} alt="Example" />} /> 
            ...
            {/**
                        ...
                        il n'est pas autorisé de modifier une facture
  ...
  return (
      ...
      <div className="facturation-container">
          ...
          <Form form={form} layout="vertical" initialValues={{ date: dayjs() }}>
                ...
                >
                  ...
                  <Radio value="OUI">
                    {isEditingFacture && editingTaxeSecondaire ? (
                      // Mode modification : afficher le taux de la facture originale
            ...
            <div className='TotalMdaleDevisFacture'>
              <div className='blocTotaleModale'>    
                <div>
                  <div>
                    {parametrage.enableMultipleTVA == 1 ? (
        ...
        >
            <div ref={factureRef} className='divModalModeleFacture'>
            <ModaleMarqueCom
                factures={facturePaye}
                onSave={handleSaveFacture}
                formRef={formRef}
                devise={devise}
                onSuccess={() => {
                  obtenirFactures();
                  setModaleFacturePayeVisible(false); // Fermer proprement la modale
                }}
              />
            </div>
        </Modal>
