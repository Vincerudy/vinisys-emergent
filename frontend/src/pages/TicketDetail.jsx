...
import { Button, Tag, List, Avatar, Typography, message, Select, Input, Steps, Modal } from "antd";
...
const TicketDetail = () => {
  ...
  const getTicketById = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/tickets/${user_id}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      try {
        const data = await getTicketById(user_id);
        setTicketState(data);
...
useEffect(() => {
  ...
  const handleSaveDescription = async () => {
    try {
      ...
      if (user_id) {
        await axios.post(
          `${import.meta.env.VITE_API_URL}/tickets/update/${user_id}`,
          payload,
          {
            headers: {
              'Content-Type': 'application/json',
      ...
      // ✅ Afficher la modale et rediriger
      Modal.success({
        title: 'Succès',
        content: 'Le ticket a été enregistré avec succès.',
        onOk: () => {
          navigate('/listes/tickets');
        },
      });
